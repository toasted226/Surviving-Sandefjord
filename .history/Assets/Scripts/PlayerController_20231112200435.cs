...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (heldItem == null || heldItem != null && !heldItem.IsUsable())
            {
                TryInteract();
            }
            else 
        ...
        if (Input.GetButtonDown("Drop") && canInteract) 
        {
            if (heldItem != null) 
            {
                DropItem();
    ...
    void TryInteract() 
    {
        ...
        {
            if (heldItem != null && cols[i].gameObject == heldItem.gameObject)
                continue;

            if (cols[i].TryGetComponent<Interactable>(out var interactable) && interactable.enabled)
            {
        ...
        if (closest != null && heldItem == null || closest != null && closest.RequiresItem())
        {
            closest.Interact(this);
            canInteract = false;
            Invoke(nameof(InteractCooldown), interactCooldown);
            Debug.Log("Have interacted!");
        }
    }

    void TryUse() 
    {
        if (heldItem != null && heldItem.IsUsable())
        {
